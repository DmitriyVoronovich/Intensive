...
const BUY_DESCRIPTION =
  'Игры для покупки временно не доступны. Попробуйте позже.';
...
export function GameInformation({ details }: GameInformationProps) {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      const isFavorite = favoritesValue.some(
        (item: GameResultType) => item.id === details.id
      );
    ...
    <>
      <div className={css.game_card}>
          ...
          <div className={css.block_game_information}>
            <div className={css.game_information}>
              {information.map((item) => {
                return (
                  <div className={css.information} key={item.title}>
                    {item.title}
                    <span className={css.style_word}>{item.value}</span>
                  </div>
                );
              })}
            </div>
        ...
        <section className={css.descriptionWrapper}>
          ...
          <p className={css.description}>{details.description_raw}</p>
        </section>
        <ScreenshotsContent />
        <div className={css.btnBuyWrapper}>
          <BaseButton buttonCategory={'favorite'} onClick={onBuyGames}>
            <DollarOutlined />
            Купить игру
          </BaseButton>
        </div>
      ...
      {isOpen && (
        <InformationModal toggle={toggleModal} description={description} />
      )}
    </>
  );
}
