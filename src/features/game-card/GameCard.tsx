...
export const GameCard = ({ card, updateFavorites }: Props) => {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      const isFavorite = favoritesValue.some(
        (item: GameResultType) => item.id === card.id
      );
  ...
  return (
    <>
      <Card className={css.container} hoverable loading={loading}>
        <>
        ...
        <div className={css.cardInformationWrapper}>
          ...
          <InformationWrapper name={'Платформы:'}>
            {platformList.map((item, index) => (
              ...
              >
                {item.platform.name}
                {onSeparation(platformList, index)}
              </div>
            ))}
          </InformationWrapper>
