...
import {GenreResultType, HistoryType, PlatformResultType, QueryParamsType} from "../../../types";
...
export const getColumnsWithProps = ({genresList, platformsList, restoreSearch}: ColumsProps): ColumnsWithProps => {
    const columns: ColumnType[] = [
            ...
            render: (_, {query}) => (
                <div>
                    {query.platforms?.map((platform) => {
                        ...
                        return (
                            <span key={platform}>{platforms?.name} </span>
                        );
                    })}
                </div>
            ),
        },
        {
            ...
            key: 'time',
            dataIndex: 'time',
            render: (_, item) => <span>{new Date(item.timestamp).toLocaleString()}</span>,
        },
        {
            title: '',
            key: 'action',
            dataIndex: 'action',
            render: (_, item) => (
                <Button onClick={() => restoreSearch(item.query)}>Перейти к поиску</Button>
            ),
        },
    ];
